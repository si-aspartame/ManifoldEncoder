...
# %%
def custom_loss(output, target, in_diff_sum, lat_diff_sum, alpha):
...
for epoch in range(1, num_epochs+1):
    ...
    for data in data_iter:
        ...
        loss = criterion(output, batch, in_diff_sum, lat_diff_sum, alpha)#batch = 入力 = 教師データ
        #print(loss)
        # ===================backward====================
        optimizer.zero_grad()
        loss.backward()
    ...
    if temp_loss < best_loss:
        ...
        es_count = 0
    es_count += 1
    alpha = temp_mse / temp_distance
    print(f'epoch [{epoch}/{num_epochs}], loss:{temp_loss}, \n g_mse = {temp_mse}, g_distance:{temp_distance}, next_alpha:{alpha}')
    all_loss.append(
...
# %%
cr = CoRanking(np_x)
print(f'{cr.evaluate_corank_matrix(result, 19683, 100)}')
...
#データ点をランダムサンプリング、1000個にして5回coranking調べて平均取る

...
def custom_loss(output, target, in_diff_sum, lat_diff_sum, out_diff_sum):
    ...
    # g_distance_1 = ((KL_divergence(SM(in_diff_sum).log(), SM(lat_diff_sum))+KL_divergence(SM(lat_diff_sum).log(), SM(in_diff_sum)))/4)
    # g_distance_2 = ((KL_divergence(SM(lat_diff_sum).log(), SM(out_diff_sum))+KL_divergence(SM(out_diff_sum).log(), SM(lat_diff_sum)))/4)
    # g_distance = (LAMBDA_1*g_distance_1)+(LAMBDA_2*g_distance_2)
    loss = g_mse+100*g_distance
    return loss
...
def do_plot(model, epoch, g_mse, g_distance):
    ...
    model.eval()
    for n, data in enumerate(DataLoader(in_data, batch_size=BATCH_SIZE, shuffle=False)):#シャッフルしない
        #print(f'TEST:{n}')
        batch = data
...
# %%
np_x = preprocessing.MinMaxScaler().fit_transform(sr)
...
# %%
in_data = torch.from_numpy(np_x.astype(np.float32))#np_xをテンソルにしたもの
print(f"in_data:{in_data.size()}")
...
for epoch in range(1, num_epochs+1):
    ...
    temp_loss = 0
    model.train()
    data_iter = DataLoader(in_data, batch_size=BATCH_SIZE, shuffle=True)
...
# %%
LATENT_DIMENSION=3
plotly.offline.iplot(plot_latent(rnd_result, rnd_color), filename='decoded swiss roll')
